...
func (v *V8Context) CallFunc(funcName string, cid int, ress []string) (res interface{}, err error) {
	ptr := C.CString(funcName[:])
	defer C.free(unsafe.Pointer(ptr))

	resarr := make([]*C.char, len(ress))
	for i := range ress {
		resarr[i] = C.CString(ress[i])
		defer C.free(unsafe.Pointer(resarr[i]))
	}

	ret := C.v8_callfunc(v.v8context, ptr, C.int(cid), (**C.char)(unsafe.Pointer(&resarr[0])), C.int(len(ress)))
